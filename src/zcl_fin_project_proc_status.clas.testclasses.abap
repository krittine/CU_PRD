...
*
*    lo_function = NEW zcl_fin_project_proc_status( ).
*
*    cl_abap_unit_assert=>assert_equals( act =  lo_function->get_name( ) exp = zcl_fin_project_proc_status=>c_function_name ) .
*    lo_tracer = NEW tcl_test_tracer( ).
*
*    " check 7 weekdays
*    lv_project = 'R.11224004'. "
*    "DO 7 TIMES.
...
*                                    tracer     = lo_tracer
*                                    parameters = VALUE #( ( name = zcl_fin_project_proc_status=>c_parameter_name value = REF #( lv_project ) ) )
*               )
*            IMPORTING
...
*                                  rule_name  = 'ZDummyRuleName'
*                                  tracer     = lo_tracer
*                                  parameters = VALUE #( ( name = zcl_fin_project_proc_status=>c_parameter_name  ) )
*             )
*          IMPORTING
*            ev_result  = lr_result
*        ).
...
*
*    " check invalid char
*    lv_project = 'CB9980000013'. " Krittin
*    TRY.
*        lo_function->execute(
*          EXPORTING
*            is_runtime = VALUE #( event_id   = 'FINS_ACC_MS_1'
...
*                                   parameters = VALUE #( ( name       =  zcl_fin_project_proc_status=>c_parameter_name
*                                                           expression = VALUE #(
*                                                           abap_type  =  CAST cl_abap_elemdescr( cl_abap_typedescr=>describe_by_name( 'ZDPROJECT' ) ) ) ) ) )
*      IMPORTING
*        ev_rc           = DATA(lv_rc)
*    ).
...
*    cl_abap_unit_assert=>assert_equals( act = xsdbool( line_exists( lt_params[ KEY p COMPONENTS name = zcl_fin_project_proc_status=>c_parameter_name ] ) ) exp = abap_true ).
*
*  ENDMETHOD.
*
